...
var instrucciones = ['Utilizar las teclas para mover las piezas.'
, 'Ordena las piezas de forma correcta para alcanzar el objetivo'];
...
function chequearSiGano() {
   ...
   if(grilla.length == grillaWin.length) {
     for(var i= 0; i < grilla.length; i++){
      for(var j= 0; j < grillaWin.length; j++){
  ...
  document.body.onkeydown = (function(evento) {
    if (evento.which === codigosDireccion.ABAJO ||
      ...
      moverEnDireccion(evento.which);
        ...
        var gano = chequearSiGano();flecha
        if (gano) {
          setTimeout(function() {
              mostrarCartelGanador();
              }, 500);
